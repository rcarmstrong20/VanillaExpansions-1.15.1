...
public class VeFrameBlock extends ContainerBlock implements IWaterLoggable
{
    ...
    private void placePainting(World world, BlockPos placePos, int height, int width, List<Map> paintingParts)
    {
        int realHeight = height - 1;
        int realWidth = width - 1;

        BlockPos bottomXCorner = placePos.down(realHeight).east(realWidth);
        BlockPos bottomZCorner = placePos.down(realWidth).south(realWidth);

        TileEntity topFrameCorner = world.getTileEntity(placePos);
     ...
     */
    private static boolean isEmpty(TileEntity tileEntity)
    {
        if (tileEntity instanceof VeFrameTileEntity)
        {
            VeFrameTileEntity frame = (VeFrameTileEntity) tileEntity;

            return frame.getInventory().get(0) == ItemStack.EMPTY;
        }
        else
        {
            return false;
    ...
    private void harvestPainting(World world, List<BlockPos> framePoses, List<Map<Item, Item>> paintingParts)
    {
        if (this.isPaintingPart(world, framePoses, paintingParts))
        {
            this.clearFrames(world, framePoses);

            world.playSound(null, framePoses.get(0), SoundEvents.ENTITY_PAINTING_BREAK, SoundCategory.BLOCKS, 1.0F,
                    0.8F + world.rand.nextFloat() * 0.4F);
        }
    }
     ...
     * harvested. It returns an exception if both lists are different sizes.
     */
